...
import de.elnarion.web.wida.metadataservice.domain.contentmetadata.*;
...
public class WidaContentMetaDataServiceImpl implements WidaContentMetaDataService, WidaErrorConstants, WidaConstants {
    ...
    private <T extends BaseItem> T getItem(CriteriaQuery<T> query) {
        T item;
        try {
        ...
        if (item.getBaseTypeId() != item.getObjectTypeId()) {
            String objectTypeId = item.getObjectTypeId();
            Long objectId = item.getId();
            Set<String> secondaryIds = item.getSecondaryTypeIds();
            item.setProperties(getNonBaseTypeProperties(objectId, objectTypeId, secondaryIds));
        }
        return item;
    }
